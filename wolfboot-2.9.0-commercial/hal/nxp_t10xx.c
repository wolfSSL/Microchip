...
static void hal_ddr_init(void)
{
    ...
    /* Map LAW for DDR - index 15.
     * On T1040, hal_pcie_init uses LAW 8-15 (4 controllers x 2 LAWs),
     * overwriting this entry. hal_init() re-establishes it after PCIe init. */
#ifdef TARGET_nxp_t1040
    set_law(15, 0, DDR_ADDRESS, LAW_TRGT_DDR_1, LAW_SIZE_8GB, 0);
#else
...
void hal_early_init(void)
{
    ...
    set32(DCFG_DEVDISR1,
        ((1 << 19) | /* Disable USB1 */
         (1 << 18) | /* Disable USB2 */
         (1 << 15)   /* SATA1 */
#ifndef TARGET_nxp_t1040
         | (1 << 2)  /* DIU (LCD) - T1024 only */
#endif
    ));

#ifndef TARGET_nxp_t1040
    set32(DCFG_DEVDISR3,
        (1 << 30) /* Disable PEX2 (PCIe2) - T1024 only */
    );
#endif
...
#ifdef TARGET_nxp_t1040
#define CONFIG_PCIE4_MEM_PHYS_HIGH      0xCULL
#define CONFIG_PCIE4_MEM_PHYS           0x30000000
#define CONFIG_PCIE4_MEM_VIRT           0xB0000000
#define CONFIG_PCIE4_IO_PHYS_HIGH       0xFULL
#define CONFIG_PCIE4_IO_PHYS            0xF8030000
#define CONFIG_PCIE4_IO_VIRT            CONFIG_PCIE4_IO_PHYS
#endif
...
static int hal_pcie_init(void)
{
    ...
    for (pcie_bus=1; pcie_bus<=PCIE_MAX_CONTROLLERS; pcie_bus++) {
        ...
        if (pcie_bus == 1) {
            ...
            mem_virt = CONFIG_PCIE4_MEM_VIRT;
            io_virt = CONFIG_PCIE4_IO_VIRT;
            mem_phys_h = CONFIG_PCIE4_MEM_PHYS_HIGH;
            mem_phys = CONFIG_PCIE4_MEM_PHYS;
            io_phys_h = CONFIG_PCIE4_IO_PHYS_HIGH;
            io_phys = CONFIG_PCIE4_IO_PHYS;
        }
#endif
        else {
            continue;
        ...
        /* LAW_TRGT_PCIE1 = 0, LAW_TRGT_PCIE2 = 1, LAW_TRGT_PCIE3 = 2 */
    ...
    /* Enumerate each PCIe controller that has an active link.
     * Check the LTSSM (Link Training and Status State Machine) register
     * to verify the link is up (L0 state = 0x16) before enumeration.
     * Enumerating a controller without a link partner can cause a
    ...
    for (pcie_bus = 1; pcie_bus <= PCIE_MAX_CONTROLLERS; pcie_bus++) {
        ...
        if (get32(DCFG_DEVDISR3) & (1 << (32 - pcie_bus)))
            continue;
        ltssm = get32(PCIE_LTSSM(pcie_bus)) & 0x3F;
        if (ltssm < PCIE_LTSSM_L0) {
            wolfBoot_printf("PCIe %d: No link (LTSSM 0x%x)\n",
                pcie_bus, ltssm);
            continue;
        }
        wolfBoot_printf("PCIe %d: Link up (LTSSM 0x%x)\n",
            pcie_bus, ltssm);
        ret = pci_enum_bus(0, &enum_info);
        if (ret != 0) {
            wolfBoot_printf("PCIe %d: Enum failed %d\n", pcie_bus, ret);
            ret = 0; /* continue with other controllers */
        }
    }
...
static void hal_mp_up(uint32_t bootpg)
{
    ...
    wolfBoot_printf("MP: Starting cores 2-%d (boot page %p, spin table %p)\n",
        CPU_NUMCORES, bootpg, (uint32_t)&_spin_table);
...
static void hal_mp_init(void)
{
    ...
    /* Assign virtual boot page at end of 32-bit addressable DDR (0x7FFFF000) */
    bootpg = (uint32_t)(DDR_ADDRESS + (DDR_SIZE > 0x80000000ULL ? 0x80000000ULL : DDR_SIZE) - BOOT_ROM_SIZE);
