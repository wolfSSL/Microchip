...
void hal_init(void)
{
    ...
    /* Make flash TLB cacheable for XIP performance (L1 I + L2 + CPC). The
     * cluster L2 runs with ECC ON (see boot_ppc_start.S), so the flash text
     * cached at 0xFFFE0000 no longer corrupts the kernel's 0x1E0000 L2 set. */
...
static void hal_mp_up(uint32_t bootpg, uint32_t spin_table_ddr)
{
    ...
    /* Release ALL cores from reset into the spin loop -- for Linux AND
     * VxWorks 7 64-bit. The earlier OS64 path held secondaries in reset on
     * the theory that CW U-Boot leaves DCFG_BRR=0 for VxWorks; the U-Boot
     * VxWorks boot disproves it (U-Boot releases CPU0/2/4/6 into the spin
     * loop, then VxWorks kicks each via cpu-release-addr). The OS can only
     * pull a core out of the spin LOOP, never out of RESET -- so with the
     * secondaries held in reset VxWorks stalls on the first release. */
...
static void hal_mp_init(void)
{
    ...
    *st = spin_table_ddr;
     ...
     * and the OS (VxWorks/Linux) inherits valid L2 lines. */
...
memory_fixup_done:
    ...
    while (off >= 0) {
        ...
        /* Mark EVERY core "okay". VxWorks (like CW U-Boot, which boots
         * with CPU Count: 8) brings up each okay core via the spin-table
         * protocol. Marking the secondaries "disabled" made VxWorks skip
         * them and then stall on the very first (CPU0) spin-table release. */
