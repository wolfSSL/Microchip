 ...
 * VDDIOxVRSEL: 0 = 3.3V range, 1 = 1.8V range (must match physical supply). */
#define PWR_SVMCR3_VDDIO2SV     (1 << 8)
#define PWR_SVMCR3_VDDIO3SV     (1 << 9)
#define PWR_SVMCR3_VDDIOVRSEL   (1 << 24)
#define PWR_SVMCR3_VDDIO2VRSEL  (1 << 25)
#define PWR_SVMCR3_VDDIO3VRSEL  (1 << 26)

/* RCC AHB5 reset registers (set = assert, clear = release) */
#define RCC_AHB5RSTSR           (*(volatile uint32_t *)(RCC_BASE + 0xA20))
#define RCC_AHB5RSTCR           (*(volatile uint32_t *)(RCC_BASE + 0x1220))
#define RCC_AHB5RSTR_XSPI2RST   (1 << 12)
#define RCC_AHB5RSTR_XSPIMRST   (1 << 14)
 ...
 */
/* XSPI2 controller registers: accessed via the NS alias regardless of
 * TZ state. The XSPI2 peripheral is left non-secure by the Boot ROM
 * (RIFSC.RISC_SECCFGR2 bit 23 = 0); accessing it via the secure alias
 * fails. Both wolfBoot (running Secure) and the NS application drive
 * the same controller through 0x4802xxxx -- a secure CPU's access to
 * an NS peripheral via the NS alias is permitted. */
#define OCTOSPI_BASE            (0x4802A000UL) /* AHB5 non-secure */
#define OCTOSPI_MEM_BASE        (0x70000000UL) /* XSPI2 mmap, S-IDAU alias */
#define OCTOSPI_MEM_BASE_NS     (0x90000000UL)
#define OCTOSPI_MEM_BASE_S      (0x70000000UL)
#define OCTOSPI_MEM_SIZE        (0x04000000UL) /* 64 MB */
...
/*** XSPI1 (control register only -- we use it to gate XSPIM_CR writes) ***/
#if TZ_SECURE()
#define XSPI1_BASE              (0x58025000UL)
#else
#define XSPI1_BASE              (0x48025000UL)
#endif
#define XSPI1_CR                (*(volatile uint32_t *)(XSPI1_BASE + 0x00))
