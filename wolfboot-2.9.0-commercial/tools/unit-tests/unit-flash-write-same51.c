...
/* hal/same51.c's hal_init() contains an ARMv7E-M-only "cpsid i" instruction
...
#include "../../hal/same51.c"

/* hal_flash_write() pokes the NVMCTRL command register (NVMCTRLB, fixed at
 * NVMCTRL_BASE == 0x41004000) directly; map that page so those writes don't
 * fault. Its contents are otherwise irrelevant here. */
...
{
    Suite *s = suite_create("flash-write-same51");
    TCase *tc = tcase_create("flash-write-same51");
