 ...
 *
 * Unit tests for the page-loop bound in hal_flash_erase() (hal/stm32g0.c).
 * Regression for F-3964: end_address = address + len - 1 (inclusive) combined
...
/* hal/stm32g0.c is tightly coupled to STM32G0 hardware registers.
...
/* DMB is a no-op on the host (stm32g0 DMB fires before STRT is written, so
...
/* Constants from hal/stm32g0.c (must mirror exactly). */
...
/* Regression for F-3964: len = PAGE_SIZE + 1 spans two pages and both must be
...
Suite *flash_erase_g0_suite(void)
{
    Suite *s  = suite_create("flash-erase-g0");
    TCase *tc = tcase_create("flash-erase-g0");
