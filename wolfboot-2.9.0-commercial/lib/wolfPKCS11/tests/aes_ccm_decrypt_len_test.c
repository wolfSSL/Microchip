...
#define CCM_LEN_TEST_DIR "./store/aes_ccm_decrypt_len_test"
#define WOLFPKCS11_TOKEN_FILENAME "wp11_token_0000000000000001"

/* MAC length configured for the operation -> minimum ciphertext length */
#define CCM_MAC_LEN  16
...
static void ccm_mech_init(CK_MECHANISM* mech, CK_CCM_PARAMS* ccmParams,
                          byte* iv, CK_ULONG ivLen)
{
    XMEMSET(iv, 9, ivLen);
    ccmParams->ulDataLen = 0;
    ccmParams->pIv       = iv;
    ccmParams->ulIvLen   = ivLen;
    ccmParams->pAAD      = NULL;
    ccmParams->ulAADLen  = 0;
    ccmParams->ulMacLen  = CCM_MAC_LEN;
...
/*
 * Test 1: C_Decrypt with ciphertext shorter than the MAC (0, 1, 15 bytes) must
 ...
 */
static int test_ccm_short_ciphertext(CK_SESSION_HANDLE session,
                                     CK_OBJECT_HANDLE key)
{
    CK_RV ret;
    CK_MECHANISM mech;
    ...
    shortLens[0] = 0;
    shortLens[1] = 1;
    shortLens[2] = CCM_MAC_LEN - 1; /* 15 */
    XMEMSET(encBuf, 0, sizeof(encBuf));

    for (i = 0; i < 3; i++) {
        ccm_mech_init(&mech, &ccmParams, iv, sizeof(iv));
...
/*
 * Test 2: Boundary case. Ciphertext of exactly macLen (16) bytes must NOT be
 ...
 */
static int test_ccm_boundary_ciphertext(CK_SESSION_HANDLE session,
                                        CK_OBJECT_HANDLE key)
{
    CK_RV ret;
    CK_MECHANISM mech;
    ...
    decSz = 0;
    ret = funcList->C_Decrypt(session, encBuf, CCM_MAC_LEN, NULL, &decSz);
    ...
    decSz = sizeof(dec);
    ret = funcList->C_Decrypt(session, encBuf, CCM_MAC_LEN, dec, &decSz);
...
static int aes_ccm_decrypt_len_test(void)
{
    CK_RV ret;
    ...
    cleanup_test_files(CCM_LEN_TEST_DIR);
    ...
    if (test_ccm_short_ciphertext(session, key) != 0)
        result = -1;
    if (test_ccm_boundary_ciphertext(session, key) != 0)
        result = -1;
...
int main(int argc, char* argv[])
{
    ...
    XSETENV("WOLFPKCS11_TOKEN_PATH", CCM_LEN_TEST_DIR, 1);
#endif

    (void)argc;
    (void)argv;

    printf("=== wolfPKCS11 AES-CCM Decrypt Length Range Test ===\n");

    (void)aes_ccm_decrypt_len_test();
...
int main(int argc, char* argv[])
{
    ...
    printf("AES-CCM not available, skipping decrypt length range test\n");
    return 0;
}
