...
static CK_RV test_token_object(void* args)
{
    CK_SESSION_HANDLE session = *(CK_SESSION_HANDLE*)args;
    CK_RV ret = CKR_OK;
...
static CK_RV test_find_objects(void* args)
{
    ...
    CK_OBJECT_HANDLE obj = CK_INVALID_HANDLE;
    static byte keyData[] = { 0x00 };
    static byte id[] = { 0x01, 0x02, 0x03, 0x04, 0x05 };
    CK_ATTRIBUTE tmpl[] = {
        { CKA_CLASS,             &privKeyClass,     sizeof(privKeyClass)      },
        { CKA_KEY_TYPE,          &genericKeyType,   sizeof(genericKeyType)    },
        { CKA_VALUE,             keyData,           sizeof(keyData)           },
...
static CK_RV rsa_pkcs15_enc_test(CK_SESSION_HANDLE session,
                                 CK_OBJECT_HANDLE priv, CK_OBJECT_HANDLE pub)
{
    CK_RV  ret = CKR_OK;
...
static CK_RV rsa_encdec_fail(CK_SESSION_HANDLE session, CK_MECHANISM* mech,
                             CK_OBJECT_HANDLE priv, CK_OBJECT_HANDLE pub)
{
    CK_RV ret;
    CK_OBJECT_HANDLE key = CK_INVALID_HANDLE;
...
static CK_RV test_rsa_x_509_fail(void* args)
{
    ...
    if (ret == CKR_OK)
        ret = get_rsa_pub_key(session, NULL, 0, &pub);
    if (ret == CKR_OK)
        ret = rsa_encdec_fail(session, &mech, priv, pub);
    if (ret == CKR_OK) {
...
static CK_RV test_ecc_gen_keys_token(void* args)
{
    ...
    if (ret == CKR_OK)
        ret = find_ecc_priv_key(sessionRO, &priv, privId, privIdLen);
    if (ret == CKR_OK)
        ret = find_ecc_pub_key(sessionRO, &pub, pubId, pubIdLen);
    if (ret == CKR_OK)
        ret = ecdsa_test(sessionRO, priv, pub);
...
static CK_RV test_aes_cbc_fail(void* args)
{
    ...
    CK_RV ret;
    CK_OBJECT_HANDLE key = CK_INVALID_HANDLE;
    CK_OBJECT_HANDLE generic = CK_INVALID_HANDLE;
    byte plain[32], enc[32], dec[32], iv[16];
    CK_ULONG plainSz, encSz, decSz, ivSz;
    CK_MECHANISM mech;
...
static CK_RV pkcs11_login(int slotId, CK_SESSION_HANDLE* session)
{
    CK_RV ret;
    int flags = CKF_SERIAL_SESSION | CKF_RW_SESSION;
...
static void pkcs11_logout(int slotId, CK_SESSION_HANDLE session)
{
    (void)slotId;
...
static TEST_FUNC testFunc[] = {
    PKCS11MTT_CASE(test_session),
    PKCS11MTT_CASE(test_op_state),
    PKCS11MTT_CASE(test_token_object),
    PKCS11MTT_CASE(test_object),
    PKCS11MTT_CASE(test_attribute),
    PKCS11MTT_CASE(test_attribute_types),
    PKCS11MTT_CASE(test_attributes_secret),
#ifndef NO_RSA
    PKCS11MTT_CASE(test_attributes_rsa),
#endif
#ifdef HAVE_ECC
    PKCS11MTT_CASE(test_attributes_ecc),
#endif
#ifndef NO_DH
    PKCS11MTT_CASE(test_attributes_dh),
#endif
    PKCS11MTT_CASE(test_find_objects),
    PKCS11MTT_CASE(test_encrypt_decrypt),
    PKCS11MTT_CASE(test_digest),
    PKCS11MTT_CASE(test_sign_verify),
    PKCS11MTT_CASE(test_recover),
...
#ifndef WC_NO_RSA_OAEP
    PKCS11MTT_CASE(test_rsa_fixed_keys_oaep),
#endif
    PKCS11MTT_CASE(test_rsa_fixed_keys_pkcs15_sig),
#ifdef WC_RSA_PSS
    PKCS11MTT_CASE(test_rsa_fixed_keys_pss),
#endif
    PKCS11MTT_CASE(test_rsa_fixed_keys_store_token),
    PKCS11MTT_CASE(test_rsa_x_509_fail),
    PKCS11MTT_CASE(test_rsa_pkcs_encdec_fail),
#ifndef WC_NO_RSA_OAEP
    PKCS11MTT_CASE(test_rsa_pkcs_oaep_encdec_fail),
#endif
    PKCS11MTT_CASE(test_rsa_pkcs_sig_fail),
#ifdef WC_RSA_PSS
    PKCS11MTT_CASE(test_rsa_pkcs_pss_sig_fail),
#endif
#ifdef WOLFSSL_KEY_GEN
    PKCS11MTT_CASE(test_rsa_gen_keys),
    PKCS11MTT_CASE(test_rsa_gen_keys_id),
#endif
#endif
#ifdef HAVE_ECC
    PKCS11MTT_CASE(test_ecc_create_key_fail),
    PKCS11MTT_CASE(test_ecc_fixed_keys_ecdh),
    PKCS11MTT_CASE(test_ecc_fixed_keys_ecdsa),
    PKCS11MTT_CASE(test_ecc_gen_keys),
    PKCS11MTT_CASE(test_ecc_gen_keys_id),
    PKCS11MTT_CASE(test_ecc_gen_keys_token),
    PKCS11MTT_CASE(test_ecdsa_sig_fail),
#endif
#ifndef NO_DH
    PKCS11MTT_CASE(test_dh_fixed_keys),
    PKCS11MTT_CASE(test_dh_gen_keys),
#endif
#ifndef NO_AES
#ifdef HAVE_AES_CBC
    PKCS11MTT_CASE(test_aes_cbc_fixed_key),
    PKCS11MTT_CASE(test_aes_cbc_fail),
    PKCS11MTT_CASE(test_aes_cbc_gen_key),
    PKCS11MTT_CASE(test_aes_cbc_gen_key_id),
#endif
#ifdef HAVE_AESGCM
    PKCS11MTT_CASE(test_aes_gcm_fixed_key),
...
#ifndef NO_HMAC
#ifndef NO_MD5
    PKCS11MTT_CASE(test_hmac_md5),
    PKCS11MTT_CASE(test_hmac_md5_fail),
#endif
#ifndef NO_SHA
    PKCS11MTT_CASE(test_hmac_sha1),
    PKCS11MTT_CASE(test_hmac_sha1_fail),
#endif
#ifdef WOLFSSL_SHA224
    PKCS11MTT_CASE(test_hmac_sha224),
    PKCS11MTT_CASE(test_hmac_sha224_fail),
#endif
#ifndef NO_SHA256
    PKCS11MTT_CASE(test_hmac_sha256),
    PKCS11MTT_CASE(test_hmac_sha256_fail),
#endif
#ifdef WOLFSSL_SHA384
    PKCS11MTT_CASE(test_hmac_sha384),
    PKCS11MTT_CASE(test_hmac_sha384_fail),
#endif
#ifdef WOLFSSL_SHA512
