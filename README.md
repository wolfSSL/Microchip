# wolfSSL Products for Microchip

This repository contains commercially licensed wolfSSL product code solely for
use with the Microchip silicon products and/or development tools with which
our wolfSSL products are integrated.

This code is only properly licensed when following the license terms in the
license file included in the root of this repository:

[LICENSE\_WOLFSSL\_MICROCHIP\_v12052025.txt](LICENSE_WOLFSSL_MICROCHIP_v12052025.txt)

### Library Version Information

This repository currently contains the following wolfSSL product versions.

|Product|Version|Notes|
|:--|:--|:--|
|wolfBoot|2.7.0 stable|Latest release of wolfBoot|
|wolfMQTT|1.21.0 stable|Latest release of wolfMQTT|
|wolfSSH|1.4.22 stable|Latest release of wolfSSH|
|wolfSSL|5.9.1 stable|Latest release of wolfSSL|

## Product Descriptions and Details

For product descriptions and details, please reference the wolfSSL product
pages for each product included here.

|Product|Product Page|
|:--|:--|
|wolfBoot|[wolfBoot Product Page](https://www.wolfssl.com/products/wolfboot/)|
|wolfMQTT|[wolfMQTT Product Page](https://www.wolfssl.com/products/wolfmqtt/)|
|wolfSSH|[wolfSSH Product Page](https://www.wolfssl.com/products/wolfssh/)|
|wolfSSL|[wolfSSL Product Page](https://www.wolfssl.com/products/wolfssl/)|

## wolfSSL Product Manuals

For detailed information on each product, please refer to the respective
product manuals.

|Product|Manual Link|
|:--|:--|
|wolfBoot|[wolfBoot Manual](https://www.wolfssl.com/documentation/manuals/wolfboot/)|
|wolfMQTT|[wolfMQTT Manual](https://www.wolfssl.com/documentation/manuals/wolfmqtt/)|
|wolfSSH|[wolfSSH Manual](https://www.wolfssl.com/documentation/manuals/wolfssh/)|
|wolfSSL|[wolfSSL Manual](https://www.wolfssl.com/documentation/manuals/wolfssl/)|

## Additional wolfSSL Microchip Examples to Explore

|Example|Link|
|:--|:--|
|wolfSSL ATECC Demos|[GitHub repo](https://github.com/wolfssl/microchip-atecc-demos)|

## Further Resources

#### wolfSSL Website
[wolfSSL Microchip PIC32 Support](https://www.wolfssl.com/docs/pic32/)

## Support

For support inquiries and questions, please email support@wolfssl.com.

